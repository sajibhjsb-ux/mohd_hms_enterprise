...
export function AuthOtpVerificationScreen({
  ...
  const [secondsLeft, setSecondsLeft] = useState(() => Math.max(0, Math.floor(initialResendAfterSec)));
  ...
  async function resend() {
    ...
    try {
      ...
      setSecondsLeft(Math.max(1, Math.floor(res.data.resendAfterSec)));
      setCode("");
      ...
      <p className="mt-6 text-center text-sm text-muted-foreground">
        ...
        {secondsLeft > 0 ? (
          <span className="font-medium tabular-nums text-foreground/80">
            Resend in {fmtCountdown(secondsLeft)}
          </span>
        ) : (
          <Button
            type="button"
