...
const postSchema = z.object({ label: z.string().min(1, "Checklist item label is required.").max(300) });
...
export const POST = withId(
  async (id, { req, user }) => {
    ...
    const item = await db.workOrderChecklistItem.create({
      data: { workOrderId: id, label: body.label, sortOrder: count },
    });
...
export const PATCH = withId(
  async (id, { req, user }) => {
    ...
    await audit({ actorId: user.id, actorEmail: user.email, action: "WO_CHECKLIST_TOGGLED", resourceType: "WORK_ORDER", resourceId: id, metadata: { code: wo.code, itemId: item.id, done: body.done } });
    return ok(updated);
  },
  { permission: PERMISSIONS.work_orders_read }
);
